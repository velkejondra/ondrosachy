...
\chapter{Úvod}
Tato maturitní práce představuje můj šachový engine a a jak jsem ho vytvořil.
Budu se snažit popsat různé techniky a alogoritmi, které šachové enginy využívají.

V~této práci nebudu popisovat pravidla šachů, budu počítat s~vaší znalostí pravidel.
Také nebudu popisovat základní datové struktury, nebo algoritmi jako je binární vyhledávání nebo hashmapa.
...
Pro představu jsem vygeneroval graf pomocí programu Graphviz\cite{graphviz}, který ukazuje jak se hra po třech tazích větví.
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Hodnocení pozic je děláno funkcí vytvořenou programátorem a nedostatky evaluační funkce jsou doháňeny prohledáváním spousty pozic do velké hloubky.
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Klíčová pro chování šachového enginu, protože na základě ní se rozhoduje v prohledávání pozic.
        \item Detaily v kapitole~\ref{evaluace}
    \end{itemize}
...
Algoritmy pro šachové enginy už existovali dřív, ale jejich použití bylo bržděno tehdejším hardwarem.
Už v 50.\ letech minulého století se znali všechny algoritmy, aby mohl být stvořen použitelný šachový engine.
...
Už i tradiční šachový engine jako je Stockfish začal používat na hodnocení pozic neronovými sítě a při vydání této změny získal více než 80 Elo bodů.\cite{stockfish_nnue}


\begin{figure}
    \centering
    \includegraphics[scale=0.5]{images/engine_evolution}
...
Při vracení rekurze porovnám všechny potomky pozice a vyberu tu nejvýhodnější pro hráče právě na tahu.
...
Napříkla jestli je kůň aktivní ve středu, nebo jestli je v podstatě k ničemu v rohu.
Proto si vytvořím pole 64 čísel pro každý typ figurky.
Pole bude obsahovat hodnoty, které budou určovat hodnotu dané figurky na daném místě.
Pro koňe vybadá toto pole takto. Viz obr.\ref{fig:heatmap}
Poté takto ohodnotím kvalitu postavení všech figurek a přičtu to k finálnímu skóre pozice.


\begin{figure}
    \centering
    \includegraphics[scale=0.8]{images/heatmap}
    \caption{Heatmapa hodnot pro koňe, vytvořená pomocí knihovny matplotlib v pythonu.}
    \label{fig:heatmap}
\end{figure}

Pro účely mého jednoduchého šachového bohatě stačí tyto dvě heuristiky, ale používá se jich mnohem více.
Například celková struktura pěšáků, či síla kombinace dvou střelců.
Evaluační funkce taky může brát ohled na to v jaké fázi hry se hra nachází.
...
Pro ilustraci přikládám screenshot GUI. Viz obr.\ref{fig:gui}

\begin{figure}
    \centering
    \includegraphics[scale=0.35]{images/gui}
...
Klíčový pro šachový je rychlost.
Proto abychom měli šachový engine co nejrychlejší musíme ho různými způsoby optimalizovat.
Například použití rychlejšího algoritmu, efektivnější datové struktury, či prosté odstranění redundantních operacích.
Je důležité primárně optimalizovat funkce, které jsou nejvíce časově náročné, abychom z našeho úsilí vytěžili co nejvíce.
Pro to využiju tzv. profiler.
Jedná se o program, který analyzuje chod programu a umožní mi zjistik kolik jaké funkce zabírají času.
Já rád používám linuxový program perf\footnote{obsažený v linuxovém kernel} s visualizací v podobě plamenového grafu\cite{flame_graphs}. Viz obr.~\ref{fig:flame_graph}
...
Z grafu~\ref{fig:flame_graph} je vidět, že nejvíc času zabíra funkce Negamax (minimax vyhledávání) a většinu této funkce zabírá zanořená funkce GetLegalMoves ( Vrací možné legální pohyby).
Takže až budu chtít ještě více optimalizovat, tak to bude generování pohybů.
Můj šachový engine zvládne pomocí Perft (viz~kapitola~\ref{subsec:debugování}) prohledat 4865609 pozic za přibližně jednu sekundu.
Pro porovnání, nejlepší volně dostupný šachový engine na světě, Stockfish stejný počet pozic prohledal za 58 milisekund, takže je ještě spousta prostoru kam můžu svůj engine zlepšovat.


\begin{figure}
    \centering
    \includegraphics[scale=0.25]{images/perf_graph}
